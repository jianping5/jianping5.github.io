...
if __name__ == '__main__':
    ...
    for file_location in sys.argv[1:]:
        ...
        # Extract path
        file_data = open(file_location, encoding='utf-8').read()
        metadata = extract_metadata(open(file_location, encoding='utf-8'), filename)
        ...
        pandoc_cmd = f'pandoc "{file_location}" -o "{temp_output_path}" {options}'
        print(f"Running: {pandoc_cmd}")
        ...
        truncated_path = os.path.split(path)[0]
        full_output_dir = os.path.join('site', truncated_path)
    ...
    for filename in os.listdir('posts'):
        if filename[-4:-1] != '.sw':
            metadatas.append(extract_metadata(open(os.path.join('posts', filename), encoding='utf-8'), filename))
    ...
    for category in categories:
        ...
        open(os.path.join('site', 'categories', category+'.html'), 'w', encoding='utf-8').write(toc)

    open('site/feed.xml', 'w', encoding='utf-8').write(feed)
    open('site/index.html', 'w', encoding='utf-8').write(make_toc(homepage_toc_items, global_config, categories))
